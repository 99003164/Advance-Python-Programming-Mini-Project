...
url = str.__add__('http://data.fixer.io/api/latest?access_key=','5934bf3f2adef071c95f87d808b3bb28')
...
class inr_usd(CurrencyConverter):
...
class MyLibrary:  # main class
    def Split():
        global book, author, copies, cost  # global variabl
        book = []
        author = []
        copies = []
        cost = []
    ...
    def Date(self):  # date method
        now=datetime.datetime.now
        return str(now().date())

    def Time(self):  # time method
        now=datetime.datetime.now
        return str(now().time())

class class_borrow(MyLibrary):  # class boorrow inherit class MyLibrary
    def __init__(self):
        pass

    def borrowBook(self):  # boorow book method
        while (True):
        ...
        while (True):
            lst = input("Borrower last name: ")   # creating output borrow file
        ...
        with open("File.txt","w+") as f:
            for var1 in range(3):
                f.write(book[var1] + "," + author[var1] + "," + str(copies[var1]) + "," + "Rs." + cost[var1] + "\n")

class returnbook(MyLibrary):  # return book class inherit MyLibray class
    def __init__(self):
        pass
    ...
    def returnBook(self):
        ...
        opt = input("Do you want to convert total amount into dollars? [y/n] ")
        if(opt == 'y' or opt == 'Y'):
            obj = inr_usd(url)
            ret = obj.conversion(total)
            fret = "{0:.2f}".format(ret)
            print("\nThe amount in USD is: ", fret,"\n")

            with open(b, "a")as f:
                f.write("Total amount in dollar: $" + str(fret))
...
object1 = MyLibrary  # object of class MyLibrary
object2 = class_borrow()  # object of class borrow
object3 = returnbook()  # object of return book
